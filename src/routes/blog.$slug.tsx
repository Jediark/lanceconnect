...
import { ArrowLeft, Twitter, Linkedin, Link2, MessageSquare, Send, Calendar, User } from "lucide-react";
...
const MOCK_COMMENTS: Record<string, Comment[]> = {
  "first-five-cold-emails": [
    { id: "c1", author: "Adebayo Alao", avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&h=100&fit=crop&q=80", date: "May 22, 2026", content: "This formula is gold. I tried the three-sentence approach this morning for a local retail shop here in Lagos and got a reply within an hour! Keeping it low pressure is key." },
    { id: "c2", author: "Sarah Jenkins", avatar: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop&q=80", date: "May 23, 2026", content: "I used to write long emails explaining my background. Now I realize nobody cares about my life story, they just want to know how I can fix their broken site." }
  ],
  "scoring-leads-without-ai": [
    { id: "c3", author: "Carlos Mendez", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop&q=80", date: "May 15, 2026", content: "I love the +15 point rule for broken contact forms. That is a massive pain point for any business because they are losing actual dollars. Brilliant checklist." }
  ],
  "pricing-as-a-new-freelancer": [
    { id: "c4", author: "Sophia V.", avatar: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&h=100&fit=crop&q=80", date: "May 8, 2026", content: "Charging by project changed my life. I used to feel rushed when coding. Now I focus purely on creating the absolute best experience for the client, and they're happy to pay a premium." }
  ]
};
      ...
      <article className="min-h-screen bg-background">
          ...
          <div className="mt-6 flex items-center gap-3 border-y border-border/60 py-4 text-sm text-muted-foreground">
            ...
            <div>
              ...
              <p className="text-xs flex items-center gap-2 mt-0.5">
                <Calendar className="h-3.5 w-3.5 text-muted-foreground/60" /> {post.date} · {post.readMins} min read
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-12 lg:grid-cols-[1fr_280px] lg:px-8">
          ...
          <div className="space-y-6">
            ...
            {headings.length > 0 && (
              <div className="rounded-xl border border-border bg-card/40 p-4 mb-6">
                <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Table of Contents</p>
                <ul className="space-y-1.5 text-sm">
                  {headings.map((h, i) => (
                    <li key={i}>
                      ...
                      >
                        <span className="text-xs text-muted-foreground/60 font-mono">0{i+1}.</span> {h}
                      </a>
                    </li>
                  ))}
                </ul>
            ...
            <div className="space-y-6 text-[16px] md:text-[17px] leading-relaxed text-foreground font-medium">
              {post.body.split("\n\n").map((para: string, i: number) => {
                ...
                if (trimmed.startsWith("- ")) {
                  ...
                  return (
                    <ul key={i} className="list-disc pl-6 my-4 space-y-2 text-foreground/90 font-normal">
                      {items.map((item, idx) => {
                        if (item.includes("**")) {
                          const parts = item.split("**");
                          return (
                            <li key={idx}>
                              {parts.map((part, pidx) => pidx % 2 === 1 ? <strong key={pidx} className="font-extrabold text-foreground">{part}</strong> : part)}
                            </li>
                          );
                        }
                ...
                return <p key={i} className="mb-4 text-foreground/90 font-normal">{para}</p>;
              })}
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="max-w-md">
                <h4 className="font-display text-lg font-bold text-foreground">Stop refreshing cold job boards.</h4>
                <p className="mt-1 text-sm text-muted-foreground">Get 10 fresh, highly-scored leads in your targeted city every week — completely free.</p>
              </div>
              <Link 
                to="/register" 
            ...
            <section className="mt-12 border-t border-border pt-10">
              ...
              <form onSubmit={handlePostComment} className="bg-card/50 rounded-xl border border-border p-6 mb-8 space-y-4">
                <h4 className="text-sm font-bold text-foreground">Join the conversation</h4>
                <p className="text-xs text-muted-foreground">Your email address will not be published. Required fields are marked *</p>
          ...
          <aside className="space-y-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Share Article</p>
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">More from our blog</p>
