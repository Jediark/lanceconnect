  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <button onClick={()=>toast.success("Export started — check your email")} className="mt-4 rounded-lg border border-border bg-background px-4 py-2 text-sm font-semibold hover:bg-accent cursor-pointer">Request export</button>
      </div>
      <div className="rounded-2xl border border-destructive/40 bg-destructive/5 p-6">
        ...
        <button onClick={handleDelete} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-destructive px-4 py-2 text-sm font-semibold text-destructive-foreground hover:opacity-90 cursor-pointer"><Trash2 className="h-4 w-4"/> Delete account</button>
      </div>
    </div>
  );
}
