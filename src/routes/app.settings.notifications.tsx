...
function NotificationsPage() {
  return (
    <form
      ...
      className="max-w-2xl space-y-3 rounded-2xl border border-border bg-card p-6 shadow-card"
    >
      {opts.map((o) => (
        <label
          key={o.id}
        ...
        >
          ...
          <input
            type="checkbox"
            defaultChecked={o.id !== "tips"}
            className="mt-1 h-4 w-4 accent-primary"
          />
        </label>
      ))}
      <button className="mt-4 rounded-lg bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
        Save preferences
      </button>
    </form>
  );
