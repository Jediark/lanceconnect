...
import { 
  Home, Search, Kanban, Mail, Sparkles, Settings as SettingsIcon, Crown, 
  Menu, X, Loader2, Sun, Moon, Bell, ChevronDown, LogOut, Briefcase
} from "lucide-react";
    ...
    <>
      <header className="sticky top-0 z-50 flex h-16 items-center gap-4 border-b border-border bg-card px-4 lg:px-8 shadow-sm">
        ...
        <nav className="hidden lg:flex items-center gap-1 flex-1">
          <NavLink to="/app/dashboard" icon={Home} label="Dashboard" active={pathname === "/app/dashboard"} />
          <NavLink to="/app/discover" icon={Search} label="Discover" active={pathname === "/app/discover"} />
          <NavLink to="/app/pipeline" icon={Kanban} label="Pipeline" active={pathname === "/app/pipeline"} />
          ...
          <div className="relative" onMouseLeave={() => setOutreachDropdownOpen(false)}>
            ...
            {outreachDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-56 rounded-xl border border-border bg-popover shadow-lg py-2 animate-in fade-in slide-in-from-top-2">
                <Link to="/app/templates" className="flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-accent transition">
                  <Mail className="h-4 w-4 text-muted-foreground" /> Email Templates
                </Link>
                <Link to="/app/ai-generator" className="flex items-center justify-between px-4 py-2 text-sm text-foreground hover:bg-accent transition">
                  <div className="flex items-center gap-3">
                    <Sparkles className="h-4 w-4 text-violet-500" /> AI Generator
                  </div>
                  <span className="rounded-full bg-amber-500/20 px-1.5 py-0.5 text-[10px] font-bold text-amber-500">PRO</span>
                </Link>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          <Link to="/app/upgrade" className="hidden lg:flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground hover:brightness-110 transition shadow-sm glow-primary">
              ...
              {profileDropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-64 rounded-xl border border-border bg-popover shadow-xl z-50 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="py-2">
                      <Link to="/app/settings" className="flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-accent transition" onClick={() => setProfileDropdownOpen(false)}>
                        <SettingsIcon className="h-4 w-4 text-muted-foreground" /> Account Settings
                      </Link>
                      <Link to="/app/settings/subscription" className="flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-accent transition" onClick={() => setProfileDropdownOpen(false)}>
                        <Briefcase className="h-4 w-4 text-muted-foreground" /> Billing & Plan
                      </Link>
                    </div>
                    <div className="p-2 border-t border-border flex items-center justify-between">
                      <button
                        onClick={() => { setTheme(theme === "dark" ? "light" : "dark"); setProfileDropdownOpen(false); }}
                      ...
                      >
                        {theme === "light" ? <><Moon className="h-3.5 w-3.5" /> Dark Mode</> : <><Sun className="h-3.5 w-3.5" /> Light Mode</>}
                      </button>
          ...
          {/* Mobile Menu Toggle */}
          <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden grid h-9 w-9 place-items-center rounded-lg border border-border bg-background">
        ...
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute inset-y-0 right-0 w-3/4 max-w-sm bg-card border-l border-border shadow-2xl animate-in slide-in-from-right">
            ...
            <div className="p-4 space-y-4">
              <MobileNavLink to="/app/dashboard" icon={Home} label="Dashboard" active={pathname === "/app/dashboard"} onClick={() => setMobileMenuOpen(false)} />
              <MobileNavLink to="/app/discover" icon={Search} label="Discover" active={pathname === "/app/discover"} onClick={() => setMobileMenuOpen(false)} />
              <MobileNavLink to="/app/pipeline" icon={Kanban} label="Pipeline" active={pathname === "/app/pipeline"} onClick={() => setMobileMenuOpen(false)} />
              <div className="pt-4 border-t border-border">
                <p className="px-3 text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Outreach</p>
                <MobileNavLink to="/app/templates" icon={Mail} label="Email Templates" active={pathname.includes("/templates")} onClick={() => setMobileMenuOpen(false)} />
                <MobileNavLink to="/app/ai-generator" icon={Sparkles} label="AI Generator" active={pathname.includes("/ai-generator")} onClick={() => setMobileMenuOpen(false)} />
              </div>
              <div className="pt-4 border-t border-border">
                <MobileNavLink to="/app/settings" icon={SettingsIcon} label="Settings" active={pathname.includes("/settings")} onClick={() => setMobileMenuOpen(false)} />
              </div>
            </div>
          </div>
        </div>
