...
export function GoalTracker({ current = 34, target = 50, className }: { current?: number, target?: number, className?: string }) {
  const radius = 40;
  ...
  return (
    <div className={cn("flex items-center gap-6 p-4 rounded-2xl border border-border bg-card shadow-sm", className)}>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">You need {Math.max(0, target - current)} more leads to hit your goal.</p>
