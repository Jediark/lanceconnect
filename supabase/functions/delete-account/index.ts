...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(supabase, user.id, ipAddress, 'delete-account', 50, 3600)
    ...
    if (profile.paystack_subscription_code && paystackSecretKey) {
      try {
        console.log(`Cancelling active Paystack subscription: ${profile.paystack_subscription_code}`)
