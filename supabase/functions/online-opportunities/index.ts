...
function mapCategoryToMuse(category: string): string[] {
  ...
  if (normalized.includes('web') || normalized.includes('software') || normalized.includes('dev') || normalized.includes('code')) {
    return ['Software Engineering', 'Computer and IT']
  }
  if (normalized.includes('design') || normalized.includes('creative') || normalized.includes('ui') || normalized.includes('ux')) {
    return ['Design']
  }
  if (normalized.includes('write') || normalized.includes('content') || normalized.includes('edit')) {
    return ['Editorial', 'Writing']
  }
  if (normalized.includes('market') || normalized.includes('seo') || normalized.includes('social')) {
    return ['Marketing', 'Advertising and PR']
  }
  if (normalized.includes('sale') || normalized.includes('biz') || normalized.includes('lead')) {
    return ['Sales']
  }
  if (normalized.includes('support') || normalized.includes('customer') || normalized.includes('assist')) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(supabase, user.id, ipAddress, 'online-opportunities', 50, 3600)
    ...
    try {
      ...
      const arbeitRes = await fetch('https://www.arbeitnow.com/api/job-board-api', { signal: AbortSignal.timeout(6000) })
      if (arbeitRes.ok) {
        ...
        for (const item of results) {
          ...
          if (category !== 'all') {
            ...
            const isMatch = normalizedTitle.includes(category.toLowerCase()) || 
    ...
    const deduplicatedJobs = jobs.filter(job => {
      const uniqueKey = job.url || `${job.title.toLowerCase().trim()}-${job.company.toLowerCase().trim()}`
    ...
    return new Response(JSON.stringify({ success: true, count: deduplicatedJobs.length, jobs: deduplicatedJobs }), {
