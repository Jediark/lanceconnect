...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(supabase, user.id, ipAddress, 'paystack-checkout', 50, 3600)
    ...
    try {
      await supabase.from('audit_log').insert({
        ...
        entity_type: 'payment',
        metadata: { plan_name: planName || '', checkout_type: checkoutType, currency, reference: paystackData.data.reference },
